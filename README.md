...
コンソールで、cc gamepc.c -o gamepcとしてコンパイルし、
...
gamepcでは、シバン（shebang;スクリプトの先頭にある#!で始まる文字列）

を付けることができます。ゲーム言語のスクリプトに、シバンを付けて実行権を付けると、
...
gamepcのコマンドプロンプトからは*ld filename.gmとして読み込んで下さい。#=1でrunです。
