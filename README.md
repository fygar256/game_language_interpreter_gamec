...
\*FM(for mode)というのを作って、\*FM 0の時、for文は普通の言語のように動作し、

\*FM 1の時、GAME言語オリジナルのように振る舞います。

しかし、筆者はGAME言語オリジナルのfor文の挙動をうろ覚えで、手元に資料がないため、

これが正しいかどうか分かりません。一応、両方行けるようにfor modeコマンドを作っておきました。
